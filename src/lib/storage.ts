...
function convertLogEntryForDB(sessionId: string, entry: LogEntry): PersistedLogEntry {
    return {
        ...
        data: entry.data ? new Uint8Array(entry.data.buffer, entry.data.byteOffset, entry.data.byteLength) : undefined,
...
export function convertDBEntryToLogEntry(dbEntry: PersistedLogEntry): LogEntry {
    return {
        ...
        data: dbEntry.data ? new DataView(dbEntry.data.buffer, dbEntry.data.byteOffset, dbEntry.data.byteLength) : undefined,
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                request.onsuccess = () => {
                    ...
                    const sessions = (request.result as SessionRecord[]).sort((a, b) => b.startTime - a.startTime);
                    resolve(sessions);
                };
